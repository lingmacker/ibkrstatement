...
绩效页面同时展示两种不能互相替代的收益口径：

- **期间口径**：回答导入报表期间实际赚了多少，优先读取 IBKR 的 `Total P/L for Statement Period`，并用 `期末净值 + 取款 − 入金 − 期初净值` 复核。
- **成本基础口径**：读取 `Realized & Unrealized Performance Summary`，展示已实现净额、当前未实现盈亏、已实现毛利、已实现毛损和盈利/亏损比。
- 成本基础盈亏分布
- 主要贡献者
- 月度收入与支出
- 已实现交易排行

成本基础口径可能包含期初持仓在报表期前已形成、但在本期才实现的浮盈，因此其合计可能与期间损益不同。账户总览和分享图中的“期间损益”使用现金流调整后的期间口径。
...
推荐从 IBKR Client Portal 导出英文或简体中文 Activity Statement：

1. 登录 IBKR Client Portal。
2. 进入 Performance & Reports -> Statements。
3. 选择 Activity Statement 并点击 Run。
4. 将 Language 设置为 English 或简体中文。
5. 将 Format 设置为 CSV。
6. 下载文件后在本项目中上传或粘贴内容。

解析器会将中文区块名、表头和关键枚举值规范化为内部英文模型，因此中英文报表使用同一套分析逻辑。
...
- 字段规范化目前覆盖英文及简体中文 IBKR Activity Statement CSV；其他语言尚未适配。
